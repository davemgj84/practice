...
/*
  ...
  Our function calculateChange should return an object which describes
  ...
  Valid denominations are as follows:
    ...
    the total amount of change for the cashier to give back. Omit any types of change that
...
const calculateChange = (total, cash) => {
  ...
  const giveChange = (totalChange) => {
    ...
    for (let i = 0; i < currency.length; i++) {
      changeObject[currency[i].name] = Math.floor(
        totalChange / currency[i].value
      );
      totalChange -= currency[i].value * changeObject[currency[i].name];
    }
...
console.log(calculateChange(4242, 5000));
console.log(calculateChange(1000, 3505));
...
// { fiveDollar: 1, twoDollar: 1, quarter: 2, nickel: 1, penny: 3 }
// { twentyDollar: 1, fiveDollar: 1, nickel: 1 }
